...
def generate_text_report(result, latency, peak_mem, output_path="data/bias_audit_report.txt"):
    ...
    report_lines = [
        ...
        "EXECUTION METRICS",
        "-----------------",
        f"- Algorithm Latency:          {latency:.6f} seconds",
        f"- Peak Memory Usage:          {peak_mem_mb:.2f} MB",
        "",
        "PEAK BIAS HOTSPOT ANALYSIS",
        "--------------------------",
        ...
        "",
        "A higher score indicates a combination of a significant Statistical Parity",
        "Difference (SPD) and a Disparate Impact (DI) ratio far from the ideal value",
        "of 1.0 (equality). Preprocessing transformations at this stage should be",
        "audited for potential bias introduction or preservation of historical",
        "disparities.",
        "",
        "WHALE POPULATION FINAL STATE (Sorted by Bias Score)",
        "--------------------------------------------------",
        # Table Header
        f"{'Whale ID':<10} | {'Position [S, T, D]':<20} | {'Fitness Score':<15} | {'Script Name':<25} | {'Transformation':<25} | {'Demographic Group'}",
        "-" * 140
    ]
